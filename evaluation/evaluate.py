...
def analyze_auction_data(input_folder, output_folder, max_bidders=10, min_bidders=2):
    """
    ...
    winning_bid_diffs = []
    participant_final_diffs = {f"provider_{i}": [] for i in range(10)}  # Track differences per participant
    ...
    for file in all_files:
        ...
        winner = final_bids.idxmin()  # Agent with the lowest final bid
        winning_price = final_bids.min()
    ...
    stats = {
        ...
        "Percentage of Agents Bidding Lowest Possible Bid": (lowest_bid_counts / (
                    len(all_files) * len(winner_counts))) * 100,
...
def evaluate(env_fn, num_games: int = 100, render_mode: str | None = None, model_path='models/test', **env_kwargs):
    ...
    config = {
        ...
        "explore": False,  # Use non-deterministic policy for evaluation
